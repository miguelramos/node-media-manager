    ...
    if(this.isOutside(dir)) {
        if(typeof cb === 'function') {
            process.nextTick(function() {
                cb(new Error("Permission denied to access folder outside home."), null);
            });
        } else {
            var error = new Error("Permission denied to access folder outside home.");

            this.emit('onRead', error, null);
        }
...
Local.prototype.isOutside = function(dir) {
    ...
    var resolve = path.resolve(this.home, path.join(this.home, dir));
